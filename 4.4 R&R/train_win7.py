...
with open(RELEVANCE_FILE, 'rb') as csvfile:
    ...
    with open(TRAININGDATA, "a") as training_file:
        ...
        for row in question_relevance:
    	    ...
    	    curl_cmd = 'curl -x "http://hg-vm-prx-sdc.t.rd.honda.com:8080" -k -s %s -u %s -d "q=%s&gt=%s&generateHeader=%s&rows=%s&returnRSInput=true&wt=json" "%s"' % (VERBOSE, CREDS, enc_question, relevance, add_header, ROWS, SOLRURL)
            if DEBUG:
                print (curl_cmd)
